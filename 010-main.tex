% *************************************************************************
\chapter[Working title: Quantum Chemistry Based Enzyme Activity Engineering and Screening.]
...
Since it is not possible to predict the effects of structural changes in an enzyme, it appears the only reasonable strategy is to carry out systematic screening studies.\footnote{By \textit{screening} we mean the construction of a large library of mutants which are inspected and compared for a specific property like activity.}
As was pointed out initially, such studies are expensive and therefore an alternative is to carry it out on the computer.
However, due to the computational effort required to reach a certain accuracy, either the computer power needs to be very large or the method has to be fast.
There is an inherent trade off in this situation.
If the approach is to start the screen with a fast, less accurate method used to narrow down the possible candidates, after identification of these initial lead structures higher level methods can be used to narrow down and further characterizse the most promising candidates.
