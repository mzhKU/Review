...
In both studies, the basic procedure consists of preparing structures for the enzyme substrate complex (``ES'') and the first intermediate (CALB: ``TI''\footnote{Tetrahedral intermediate}, BCX: ``GE''\footnote{Glycosyl enzyme}) and calculating the reaction barrier between these two pairs of structures, respectively.
The reaction barrier is calculated by preparing intermediate structures of the model along the reaction coordinate ES$_\text{CALB}\rightarrow$TI or ES$_\text{BCX}\rightarrow$GE (around 10 intermediate structures, Fig. \ref{fig:calb_reaction}).
The quality of the results of this approach are strongly dependent on how careful the modeling of each of these structures is carried out, a major reason for meaningless results is significant structural rearrangement during the geometry optimization.
...
Again only the rate determining step is modeled, Fig. \ref{fig:bcx_mechanism}.
\begin{figure}[htbp] 
\centering
...
In simple terms, forcing the quantum chemical model to do something it would not if unconstrained, would result in an increased reaction barrier and therefore it is advisable to keep the number of constraints as low as possible.\\
\textbf{Molecular modeling procedure and calculation parameters.}
In a second step, the modeling procedure is worked out.
A PDB structure using a covalent inhibitor is available (1BVV) and is used as a starting point for the modeling procedure because it gives a very clear indication of how the substrate is located in the active site.
...
Furthermore, ambiguity in how the substrate should be placed in the active site can be reduced using the structure including the inhibitor, which is also increases reproducibility of the results.
...
In the next step, an assessment of parameter depencendy of optimized structure energies reveals that within the given framework of approximations, conclusive results are obtained when the gradient convergence criterion is set to 1 kcal/(mol\AA) and an NDDO cutoff of 15 \AA \mbox{ i}s used.
...
Then, it is evaluated how the results depend on the size of the model included in the optimization calculations.
...
\end{minipage}
\caption{
\textbf{A}: Illustration of BCX with bound ONP substrate.
...
The substrate is visible in the center of the figure.
\textbf{B}: Calculated reaction barriers for different layers of optimized residues.\cite{hediger2013computational}
}
...
These experiments allowed to clearly justify the selected quantum chemical model (PM6) and its parametric configuration, the sequence of modeling steps in order to carry out the calculations and the layer thickness of optimized residues around the active site.\\
In the final step, using PYMOL\cite{PyMOLu}, systematically all possible single mutants within a given distance of the substrate are prepared\footnote{The PYMOL script used for the preparation of the mutants is available at https://github.com/mzhKU/Enzyme-Screening/blob/master/vsc-bcx.py.}.
Analysis of reaction barriers reveals that, after combining the best single mutants from all mutated positions, the set of double mutants on average has lower activation barriers than the set of single mutants.
